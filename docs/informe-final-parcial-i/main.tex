...
\begin{tikzpicture}[remember picture,overlay]
  ...
  % Emblema Don Gato dibujado en TikZ.
  \begin{scope}[shift={([yshift=8.35cm]card.center)}]
    \fill[coffeeInk] (0,0) circle (1.18);
    \fill[coffeePanelLight] (0,0) circle (1.02);
    \draw[coffeeInk,line width=1.1pt] (0,0) circle (1.02);
    \fill[coffeeInk] (-.45,.05) -- (-.88,.72) -- (-.72,-.05) -- cycle;
    \fill[coffeeInk] (.45,.05) -- (.88,.72) -- (.72,-.05) -- cycle;
    \fill[coffeeInk] (0,-.10) ellipse (.68 and .56);
    \fill[coffeePanelLight] (-.25,.05) circle (.08);
    \fill[coffeePanelLight] (.25,.05) circle (.08);
    \fill[coffeePanelLight] (0,-.14) circle (.055);
    \draw[coffeePanelLight,line width=.35pt] (-.05,-.20) -- (-.18,-.32);
    \draw[coffeePanelLight,line width=.35pt] (.05,-.20) -- (.18,-.32);
    \draw[coffeePanelLight,line width=.35pt] (-.10,-.08) -- (-.70,.10);
    \draw[coffeePanelLight,line width=.35pt] (-.10,-.15) -- (-.70,-.10);
    \draw[coffeePanelLight,line width=.35pt] (.10,-.08) -- (.70,.10);
    \draw[coffeePanelLight,line width=.35pt] (.10,-.15) -- (.70,-.10);
    \draw[coffeeButton,line width=2.2pt] (-.45,-.66) .. controls (-.12,-.92) and (.12,-.92) .. (.45,-.66);
    \fill[coffeeButton] (-.18,-.78) circle (.14);
    \fill[coffeeButton] (.18,-.78) circle (.14);
    \draw[coffeeInk,line width=.7pt] (-.72,.78) .. controls (-.28,1.14) and (.28,1.14) .. (.72,.78);
    \draw[coffeeInk,line width=.7pt] (-.50,.94) .. controls (-.18,1.23) and (.18,1.23) .. (.50,.94);
  \end{scope}
...
\midrule
Cobertura & 91.5\% & JaCoCo local: 90.77\% en lineas y 92\% en instrucciones. & Cumple el umbral.\\
Latencia & 185 ms & Plan JMeter orientado a validar respuestas menores a 200 ms. & Registro compatible.\\
Vulnerabilidades & 0 criticas & OWASP Dependency Check configurado como gate. & Requiere reporte.\\
Pipeline & CI/CD operativo & Flujo DevSecOps documentado para GitHub Actions. & Requiere captura o enlace.\\
Avance PDCA & 100\% por etapa & Informe cerrado con resultados y acciones de mejora. & Coherente.\\
\bottomrule
\end{tabularx}
\end{table}

La diferencia entre 91.5\% y 90.77\% no modifica la conclusion, ya que ambos valores superan el umbral definido de 85\%. Para el calculo del Quality Score se utiliza 90.77\%, por ser el dato exacto disponible en el reporte JaCoCo local.
...
La evidencia local fue generada el 13 de mayo de 2026 mediante la ejecucion de pruebas del backend. Los reportes de Surefire y JaCoCo indican que el proyecto cumple el umbral de cobertura establecido y no presenta fallos funcionales en la bateria automatizada disponible.
...
Vulnerabilidades criticas & El umbral definido es 0 criticas; OWASP Dependency Check queda configurado para el ciclo \texttt{verify}.\\
...
El Quality Scoring Engine pondera cuatro factores de McCall: Testability, Correctness, Maintainability e Integrity. El calculo usa cobertura de lineas como medida de testabilidad, resultados de pruebas como medida de correctitud, el escenario de code smells como referencia de mantenibilidad y el criterio de cero vulnerabilidades criticas como integridad.
...
Maintainability & 76.00 & 0.20 & 15.20\\
Integrity & 100.00 & 0.20 & 20.00\\
\midrule
\textbf{Total} & & & \textbf{92.43 / 100}\\
\bottomrule
\end{tabular}
\end{table}

El resultado de 92.43/100 ubica al MVP en un nivel alto para el alcance academico del proyecto. La mejora principal no esta en la ejecucion de pruebas, que ya es estable, sino en aumentar cobertura de ramas, ejecutar el ciclo completo \texttt{mvnw verify} en CI y adjuntar evidencias visuales de pipeline, OWASP, JMeter y SonarQube cuando esten disponibles.
...
La evidencia de pipeline se mantiene como parte del flujo DevSecOps propuesto. Para convertirla en evidencia final de ejecucion se requiere adjuntar una captura o enlace de GitHub Actions donde conste la corrida exitosa del build, pruebas, cobertura y analisis de seguridad.
...
Aunque el MVP cumple el objetivo academico, existen limitaciones que deben considerarse en una evolucion posterior. La cobertura de ramas es menor que la cobertura de lineas, por lo que conviene fortalecer pruebas sobre caminos alternos, errores de validacion y excepciones de seguridad. Ademas, OWASP y JMeter estan incorporados como parte del flujo de calidad, pero el informe final debe respaldarlos con ejecuciones completas del ciclo \texttt{verify} y sus reportes asociados.

Otra limitacion es que el dashboard SonarQube requiere integracion real con token y servidor o SonarCloud. Sin esa corrida, los valores asociados a quality gate deben tratarse como seguimiento academico o evidencia pendiente, no como resultado automatizado definitivo. Finalmente, el MVP todavia no incluye paginacion avanzada, roles finos por perfil ni documentacion OpenAPI generada automaticamente.
...
Para fortalecer el proyecto se recomienda integrar SonarCloud o SonarQube real en GitHub Actions, ejecutar \texttt{mvnw verify} como gate obligatorio antes de cada merge, publicar artifacts de JaCoCo, OWASP y JMeter en cada corrida de CI, e incorporar OpenAPI/Swagger como documentacion viva de endpoints.
